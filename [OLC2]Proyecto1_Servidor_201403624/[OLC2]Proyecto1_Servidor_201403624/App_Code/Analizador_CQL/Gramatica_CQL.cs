...
namespace _OLC2_Proyecto1_Servidor_201403624.Controllers.Analizador_CQL
{
    public class Gramatica_CQL:Grammar
    {
        ...
        public Gramatica_CQL() : base(caseSensitive: false)
        {
            ...
            RegisterOperators(1, Associativity.Right,PUNTO);
            RegisterOperators(2, Associativity.Right, NEGACION);
            RegisterOperators(3, SUMALOGICA, SUMALOGICAEXCLUSIVA,MULLOGICA);
            RegisterOperators(4, MAYORIGUAL, MENORIGUAL,MENOR,MAYOR,DIFERENTE,IGUALIGUAL);
            RegisterOperators(5, MAS, MENOS);
            RegisterOperators(6, MUL, DIV,MOD);
            RegisterOperators(7, POTENCIA);
            ...
            NonTerminal sentenciaasignacion = new NonTerminal("sentneciaasignacion");
            ...
            NonTerminal operacionesunarias = new NonTerminal("operacionesunarias");
            NonTerminal tipo = new NonTerminal("tipo");
            NonTerminal sentenciacrearusertype = new NonTerminal("sentenciacrearusertype");
            NonTerminal declaracionvariables = new NonTerminal("declaracionvariables");
            NonTerminal listavariables = new NonTerminal("listavariables");
            NonTerminal declaracion = new NonTerminal("declaracion");
            NonTerminal declaracionlist = new NonTerminal("declaracionlist");
            NonTerminal declaracionmap = new NonTerminal("declaracionmap");
            NonTerminal declaracionset = new NonTerminal("declaracionset");
            NonTerminal declaracionobjeto = new NonTerminal("declaracionobjeto");
            ...
            Lista_parametros.Rule = Lista_parametros + COMA + declaracion_parametro
            ...
            sentenciacrearusertype.Rule = CREAR + TYPE + verificacionExiste + ABRIRPARENTESIS + listaatributos + CERRARPARENTESIS + PUNTOCOMA
                                         | CREAR + TYPE + ABRIRPARENTESIS + listaatributos + CERRARPARENTESIS + PUNTOCOMA; ;


            listaatributos.Rule = listaatributos + COMA + declaracionesusertypes
                                | declaracionesusertypes;

            declaracionesusertypes.Rule = IDENTIFICADOR + tipo
            ...
            sentenciabatch.Rule = BEGIN + BATCH + instrucciones + APPLY + BATCH;

            sentenciacreartabla.Rule = CREAR + TABLA + verificacionExiste+ABRIRPARENTESIS+lista_declaraciones_tabla+CERRARPARENTESIS+PUNTOCOMA;
            ...
            llave_primaria_compuesta.Rule = llave_primaria_compuesta+COMA+IDENTIFICADOR
                                            |IDENTIFICADOR
                                            ;
            ...
            lista_identificadores.Rule = lista_identificadores +COMA + IDENTIFICADOR
                                        | lista_identificadores + COMA+ tipo+IDENTIFICADOR
                                        | IDENTIFICADOR
                                        | tipo+IDENTIFICADOR;
            ...
            sentenciaforeach.Rule = PARA + EACH + ABRIRPARENTESIS +Lista_parametros+ CERRARPARENTESIS + ARROBA + IDENTIFICADOR + LISTAINSTRUCCIONES;

            sentenciainsertar.Rule = INSERT + INTO + IDENTIFICADOR + VALUES + ABRIRPARENTESIS +Listavalores+ CERRARPARENTESIS + PUNTOCOMA
                                    | INSERT + INTO + IDENTIFICADOR + VALUES + ABRIRPARENTESIS + Listavalores + CERRARPARENTESIS + PUNTOCOMA;
            ...
            sentenciaeliminar.Rule = ELIMINAR + DESDE + IDENTIFICADOR + sentenciawhere
                                    |ELIMINAR +expresion+ DESDE + IDENTIFICADOR + sentenciawhere
                                    | ELIMINAR + DESDE + IDENTIFICADOR
                                    | ELIMINAR +expresion+ DESDE + IDENTIFICADOR;
            ...
            LISTA_SELECCION.Rule = LISTA_SELECCION + COMA+ IDENTIFICADOR
                                | IDENTIFICADOR
                                | MUL;

            ASIGNACIONES.Rule = ASIGNACIONES + COMA + sentenciaasignacion
                                |sentenciaasignacion;
            ...
            extras_where.Rule = MakePlusRule(extras_where, null, extra);

            extra.Rule = ORDER + BY + Lista_campos
                        | LIMIT + expresion
                        | IN + ABRIRPARENTESIS + Listavalores + CERRARPARENTESIS;

            sentenciaagregacion.Rule = TIPOAGREGACION + ABRIRPARENTESIS + MENOR + sentenciaconsulta + MAYOR + CERRARPARENTESIS;
            TIPOAGREGACION.Rule = AVG
                                | COUNT
                                | MIN
                                | MAX
                                | SUM;
            Lista_campos.Rule = Lista_campos + COMA + campo
                               | campo;
            ...
            Listacasos.Rule = CASO + expresion + DOSPUNTOS + instrucciones + Listacasos
                            | CASO + expresion + DOSPUNTOS + instrucciones
                            | DEFECTO + DOSPUNTOS + instrucciones
                            ;
            sentenciafor.Rule = PARA + ABRIRPARENTESIS + declaracion + PUNTOCOMA + expresion + PUNTOCOMA + expresion + CERRARPARENTESIS + LISTAINSTRUCCIONES;
            ...
            Listavalores.Rule = Listavalores + COMA + expresion
                                | expresion;

            sentenciadeclaracion.Rule=tipo + listavariables + PUNTOCOMA;

            listavariables.Rule = listavariables +COMA+ declaracion
                                  | declaracion;
            ...
            expresion.Rule = expresion + SUMALOGICA + expresion
                           ...
                           | ABRIRLLAVE + Listavalores + CERRARLLAVE
            ...
            OPERACIONPOST.Rule = Lista_acceso + INC
                                | Lista_acceso + DEC
                                ;
            operacionesunariomenos.Rule = MENOS + expresion;
